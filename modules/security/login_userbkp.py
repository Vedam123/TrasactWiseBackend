...
def login():
    ...
    for user_info in APPLICATION_CREDENTIALS:
        if user_info["username"] == username and bcrypt.checkpw(
            ...
            print("The User is in the Password Pair list in the config file : credential are matched")
    ...
    query = "SELECT username, password, emailid, empid FROM adm.users WHERE username = %s"
    values = (username,)
