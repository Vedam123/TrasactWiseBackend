...
def create_sales_order_header():
    ...
    try:
        ...
        logger.debug(
            f"{USER_ID} --> {MODULE_NAME}: Entered the 'create sales order header' function")
       ...
       # Perform the insertion
            ...
            logger.debug(
                f"{USER_ID} --> {MODULE_NAME}: Successfully created sales order header")
        ...
        logger.error(
            f"{USER_ID} --> {MODULE_NAME}: Error creating sales order header - {str(e)}")
        mydb.rollback()
