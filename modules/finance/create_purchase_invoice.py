...
def create_purchase_invoice():
    try:
        ...
        # Log entry point
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Entered the 'create_purchase_invoice' function")
        ...
        try:
            ...
            logger.info(f"{USER_ID} --> {MODULE_NAME}: Purchase invoice data created successfully")
            mycursor.close()
            mydb.close()
            ...
            logger.error(f"{USER_ID} --> {MODULE_NAME}: Unable to create purchase invoice data: {str(e)}")
            mycursor.close()
            mydb.close()
