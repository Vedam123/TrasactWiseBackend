...
def update_sales_invoice():
    ...
    try:
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Entered the 'update_sales_invoice' function")
        ...
        try:
            ...
            if mycursor.rowcount > 0:
                # Log success
                logger.info(f"{USER_ID} --> {MODULE_NAME}: Updated sales invoice")
            ...
            logger.error(f"{USER_ID} --> {MODULE_NAME}: Unable to update sales invoice: {str(e)}")
            mycursor.close()
            mydb.close()
