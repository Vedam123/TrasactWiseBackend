...
def get_sales_invoice_details():
    ...
    try:
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Entered the 'get_sales_invoice_details' function")
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Successfully retrieved sales invoice details data")

        return jsonify({'sales_invoice_headers': sales_invoice_headers})

    except Exception as e:
        logger.error(f"{USER_ID} --> {MODULE_NAME}: Error retrieving sales invoice details data - {str(e)}")
        import traceback
