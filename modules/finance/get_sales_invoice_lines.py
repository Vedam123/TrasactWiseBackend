...
def get_sales_invoice_lines():
    ...
    try:
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Entered the 'get_sales_invoice_lines' function")
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Successfully retrieved sales invoice lines data")

        return jsonify({'sales_invoice_lines': sales_invoice_lines})

    except Exception as e:
        logger.error(f"{USER_ID} --> {MODULE_NAME}: Error retrieving sales invoice lines data - {str(e)}")
        import traceback
