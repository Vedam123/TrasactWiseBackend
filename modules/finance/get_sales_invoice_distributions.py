...
def get_sales_invoice_accounts():
    ...
    try:
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Entered the 'get_sales_invoice_accounts' function")
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Successfully retrieved sales invoice accounts data")

        return jsonify({'sales_invoice_accounts': sales_invoice_accounts})

    except Exception as e:
        logger.error(f"{USER_ID} --> {MODULE_NAME}: Error retrieving sales invoice accounts data - {str(e)}")
        import traceback
