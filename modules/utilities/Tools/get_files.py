...
def get_files():
    try:
        ...
        logger.debug(f"{appuser} --> {__name__}: Entered the 'get files' function")
        ...
        # Collect all files in the Product Info directory
        files_data = []
        for root, dirs, files in os.walk(product_info_dir):
            for file in files:
                ...
                files_data.append({
                    'filename': file,
                    'content': content.decode('utf-8', errors='ignore')  # Decode binary content as text
                })

        logger.debug(f"{appuser} --> {__name__}: Successfully retrieved files from 'Product Info' directory")

        return jsonify({'files': files_data})

    except Exception as e:
        logger.error(f"{appuser} --> {__name__}: Error retrieving files - {str(e)}")
