...
def get_bins():
    ...
    try:
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Entered the 'get bins' function")
        ...
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Successfully retrieved bin data")

        return jsonify({'bin_list': bin_list})

    except Exception as e:
        logger.error(f"{USER_ID} --> {MODULE_NAME}: Error retrieving bin data - {str(e)}")
