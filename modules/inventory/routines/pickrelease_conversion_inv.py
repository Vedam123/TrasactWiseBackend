...
def pickrelease_conversion_inv(sales_order_lines, sales_header_id, mydb, USER_ID, MODULE_NAME, created_by, updated_by):
    try:
        logger.info(f"{USER_ID} --> {MODULE_NAME}: Entered pickrelease_conversion_inv function")

        for sales_order_line in sales_order_lines:
            logger.debug(f"{USER_ID} --> {MODULE_NAME}: Processing Sales Order Line: {sales_order_line}")
            ...
            result, status_code = uom_conversion(input_params['sales_uom_id'], input_params['sales_line_quantity'], mydb, USER_ID, MODULE_NAME)
            ...
            if status_code == 200:
                ...
                logger.debug(f"{USER_ID} --> {MODULE_NAME}: Issue in conversion: {result}")
            ...
            result, status_code = pickrelease_update_inv(input_params, result_params, mydb, USER_ID, MODULE_NAME, created_by, updated_by)

            if status_code != 200:
                logger.debug(f"{USER_ID} --> {MODULE_NAME}: Issue in updating inventory: {result}")
