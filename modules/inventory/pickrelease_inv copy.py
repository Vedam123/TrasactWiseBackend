        ...
        for sales_order in sales_orders:
            ...
            for line in sales_order_lines:
                ...
                result, status_code = allocate_inventory(line, execution_id,sales_header_id, look_only_inventory_ids,
                                                         full_qty_alloc_status, part_qty_alloc_status, mydb, current_userid,
                                                           MODULE_NAME, current_userid, current_userid)
                if status_code == 200:
...
def allocate_inventory(line, execution_id,sales_header_id, look_only_inventory_ids,full_qty_alloc_status,part_qty_alloc_status, mydb, current_userid, MODULE_NAME, created_by, updated_by):
    try:
        ...
        for inventory in available_inventory:
            ...
            if convertable_quantity_lb <= (required_quantity - total_allocated):
                ...
                response_json, status_code =update_inventory(execution_id,inventory, allocated_quantity_in_base_uom, sales_header_id, 
                                                             sales_order_line_id, sales_line_status,mydb, current_userid, MODULE_NAME)
                ...
                if remaining_quantity > 0:
                    ...
                    response_json, status_code = create_new_inventory_row(execution_id,inventory, allocated_quantity_in_base_uom, lowest_base_unit, sales_header_id, 
                                                                          sales_order_line_id, sales_line_status,mydb, current_userid, MODULE_NAME)
        ...
        if total_allocated > 0 : 
            result, status_code = update_sales_order_lines_status(execution_id,sales_header_id,sales_order_line_id,full_qty_alloc_status,part_qty_alloc_status, 
                                                                  total_allocated, mydb, current_userid, MODULE_NAME)
            ...
            if status_code == 200:
                ...
                #log_pick_release(execution_id, sales_header_id, sales_order_line_id, sales_item_id,
                #    sales_uom_id, sales_uom_id_quantity, sales_base_uom_id,
                #    sales_base_uom_quantity, already_picked_quantity, sales_line_status,
                #    status, new_inventory_id, total_allocated, 'RELEASED',
                #    created_by, mydb)
...
def update_sales_order_lines_status(execution_id, sales_header_id, sales_order_line_id, full_qty_alloc_status, 
                                    part_qty_alloc_status, total_allocated, mydb, current_userid, MODULE_NAME):
    try:
...
def update_sales_order_status(sales_header_id, full_qty_alloc_status, part_qty_alloc_status, mydb, current_userid, MODULE_NAME):
    try:
        ...
        if new_status is not None:  # Execute update only if new_status is not None
            ...
            return jsonify(message='Sales order status updated successfully'), 200
        else:
            logger.debug(f"No status update needed for Sales Header ID {sales_header_id} as new_status is None")
            return jsonify(message='No status update needed'), 200

    except Exception as e:
        logger.error(f"Error occurred during sales order status update: {str(e)}")
        return jsonify(message='Sales order status update failed'), 500
    finally:
        mycursor.close()
...
def get_latest_sales_order_line_info(header_id, line_id, mydb):
    try:
...
def log_pick_release(execution_id, sales_header_id, sales_order_line_id, sales_line_prev_status, 
    ...
    try:
        # Fetch necessary data using get_latest_sales_order_line_info function
        sales_order_line_data, status_code = get_latest_sales_order_line_info(sales_header_id, sales_order_line_id, mydb)
