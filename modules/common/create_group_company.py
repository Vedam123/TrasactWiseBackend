...
def create_group_company():
    try:
        ...
        # Log entry point
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Entered in the create group company function")
        ...
        # Log parsed data
        logger.debug(f"{USER_ID} --> {MODULE_NAME}: Parsed Legal Entity ID: {legal_entity_id}")
        ...
        try:
            ...
            logger.info(f"{USER_ID} --> {MODULE_NAME}: Group company data created successfully")
            mycursor.close()
            mydb.close()
            return jsonify({'message': 'Group company data created successfully'})
        except Exception as e:
            # Log the error and close the cursor and connection
            logger.error(f"{USER_ID} --> {MODULE_NAME}: Unable to create group company data: {str(e)}")
            mycursor.close()
            mydb.close()
